...
- **ΔT** (`delta_t.json`): asserted against backend `swe.deltat()` directly.
  ...
  their data lands.
- **Planetary longitudes** (`planet_longitudes.json`, awaiting data — see
  `ACQUISITION.md`): asserted against both engines independently
  (`test_anchors.py` + `test/anchors.test.ts` pick the file up when present).
- **Eclipses, ayanamsa, solar events**: schemas and acquisition steps in
  `ACQUISITION.md`; runners land with the data.
