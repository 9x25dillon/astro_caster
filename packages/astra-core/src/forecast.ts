...
// Per-pair orb state: [prev_orb, was_decreasing]. The trend bit detects an
// exactness pass by the sign change of the orb's derivative — the orb was
// shrinking and has now started to grow. (The old fixed +0.03°/day hysteresis
// could never fire for slow outer-planet pairs, whose orb changes ≪ 0.03°/day
// near the minimum; backend forecast.py mirrors this exactly.)
...
function minimumStep(state: OrbState, key: string, curr: number, threshold: number): number | null {
...
export function generateForecast(
  ...
  for (let i = 0; i < bodyNames.length; i++) {
    ...
    for (let j = 0; j < bodyNames.length; j++) {
      ...
      for (const [asp, ang] of ASPECTS) ttOrbs.set(`${n1}|${n2}|${asp}`, [orb(lon1, lon2, ang), true]);
    }
    for (const [nn, nlon] of Object.entries(natal)) {
      for (const [asp, ang] of ASPECTS) tnOrbs.set(`${n1}|${nn}|${asp}`, [orb(lon1, nlon, ang), true]);
    }
  }
  if ("Moon" in prevPos) {
  ...
  for (let offset = 0; offset < days; offset++) {
    ...
    // including the Sun, whose Moon aspects are the lunations (new/full Moons
    // and quarters). An index guard copied from the symmetric daily loop used
    // to skip the Sun entirely.
