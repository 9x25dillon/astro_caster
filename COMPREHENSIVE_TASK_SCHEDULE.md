...
**ID** | **Task** | **Size** | **AC / Done When** | **Deps / Notes**
---|---|---|---|---
**PDF-1** | PDF renderer for the deluxe edition | L | `report_markdown` → styled PDF per `docs/ASTRO_ARCANA_PERSONAL_REPORT_DESIGN.md` (serif, amethyst/gold/cream, pull-quotes, sigil slots, `{{SIGIL}}`/`{{BIRTH_INFO}}` filled client-side); printable mock (`docs/Astro_Arcana_Report_Design_Mock.html`) is the visual contract; works offline for the deterministic edition | Client-side render keeps birth data local (privacy invariant). Candidate: print-CSS route from the HTML mock before adding any lib.
...
**R1** | Rate limiting on paid AI paths (pulled forward) | M | 429s on `/api/oracle-report` + `/api/personal-report` + `/api/ai-ask*`; env-configurable; tests | **Do before wide traffic** — two minutes-long Fable endpoints now exist; cost exposure is real.
